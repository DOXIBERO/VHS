...
const userDataDir = 'C:\\temp\\edge_test_rot_' + Date.now();
...
try {
  ...
  await send('Page.navigate', { url: 'http://localhost:5196/VHS/' });
  ...
  const info = await send('Runtime.evaluate', {
    expression: `(() => {
      ...
      return {
        headNubPos: headNub.position,
        headNubRot: { x: headNub.rotation.x, y: headNub.rotation.y, z: headNub.rotation.z },
        headNubQuat: headNub.quaternion,
        chestPos: chest.position,
        chestRot: { x: chest.rotation.x, y: chest.rotation.y, z: chest.rotation.z },
        chestQuat: chest.quaternion
      };
    })()`,
    returnByValue: true
  });

  console.log('Bone local transforms:', JSON.stringify(info.result?.value, null, 2));
