...
const userDataDir = 'C:\\temp\\edge_officer_clean_' + Date.now();
...
try {
  ...
  await send('Page.navigate', { url: 'http://localhost:5192/VHS/' });
  ...
  // Inject Officer Skin with CLONED materials and clean accessories
  const evalResult = await send('Runtime.evaluate', {
    expression: `(() => {
      try {
        ...
        const MeshStandardMat = bodyMat.constructor;
        ...
        const navyMat = new MeshStandardMat({ color: 0x0F172A, roughness: 0.55, metalness: 0.05 });
        const sunglassesMat = new MeshStandardMat({ color: 0x050505, roughness: 0.05, metalness: 0.95 });
        const silverMat = new MeshStandardMat({ color: 0xE2E8F0, roughness: 0.1, metalness: 0.95 });
        ...
        // Sitting firmly and stylishly on the crown of the head
        // =========================================================
        const cap = new Group();
        cap.name = 'PoliceCap';

        // 1. Navy Flared Crown
        const crown = new Mesh(new CylinderGeo(0.35, 0.28, 0.15, 24), navyMat);
        crown.position.set(0, 0.84, 0.03);
        crown.rotation.x = -0.12;
        crown.castShadow = true;
        cap.add(crown);

        // 2. Gold Piping around top seam
        const piping = new Mesh(new TorusGeo(0.35, 0.012, 10, 24), goldMat);
        piping.position.set(0, 0.91, 0.04);
        piping.rotation.x = Math.PI / 2 - 0.12;
        cap.add(piping);

        // 3. Black Gloss Curved Visor (Visière)
        const visor = new Mesh(new CylinderGeo(0.32, 0.32, 0.02, 24, 1, false, 0, Math.PI), blackGlossMat);
        visor.position.set(0, 0.77, 0.14);
        visor.rotation.x = 0.40;
        ...
        const cord = new Mesh(new TorusGeo(0.30, 0.015, 8, 24, Math.PI), goldMat);
        cord.position.set(0, 0.79, 0.09);
        cord.rotation.x = Math.PI / 2 - 0.08;
        cord.rotation.z = Math.PI;
        cap.add(cord);

        // 5. Golden Police Star Crest on Front
        const badgePlate = new Mesh(new CylinderGeo(0.06, 0.06, 0.012, 16), goldMat);
        badgePlate.position.set(0, 0.86, 0.34);
        badgePlate.rotation.x = Math.PI / 2 - 0.12;
        cap.add(badgePlate);

        const star = new Mesh(new OctahedronGeo(0.04, 0), goldMat);
        star.position.set(0, 0.86, 0.36);
        ...
        // Positioned across the eyes
        // =========================================================
        const aviators = new Group();
        aviators.name = 'Aviators';

        const lensL = new Mesh(new BoxGeo(0.15, 0.11, 0.02), sunglassesMat);
        lensL.position.set(-0.10, 0.58, 0.37);
        lensL.rotation.y = -0.12;
        aviators.add(lensL);

        const lensR = new Mesh(new BoxGeo(0.15, 0.11, 0.02), sunglassesMat);
        lensR.position.set(0.10, 0.58, 0.37);
        lensR.rotation.y = 0.12;
        aviators.add(lensR);

        // Gold Top Brow Bar & Bridge
        const browBar = new Mesh(new CylinderGeo(0.008, 0.008, 0.36, 8), goldMat);
        browBar.rotation.z = Math.PI / 2;
        browBar.position.set(0, 0.63, 0.37);
        aviators.add(browBar);

        const bridge = new Mesh(new CylinderGeo(0.008, 0.008, 0.06, 8), goldMat);
        bridge.rotation.z = Math.PI / 2;
        bridge.position.set(0, 0.59, 0.38);
        aviators.add(bridge);
        ...
        const chestBadge = new Mesh(new OctahedronGeo(0.065, 0), goldMat);
        chestBadge.position.set(-0.16, 0.28, 0.43);
        chestBadge.scale.set(1, 1.2, 0.35);
        chestBadge.castShadow = true;
        bodyDetails.add(chestBadge);

        // 2. Black Police Tie
        const tie = new Mesh(new BoxGeo(0.075, 0.30, 0.02), blackGlossMat);
        tie.position.set(0, 0.20, 0.44);
        tie.rotation.x = -0.10;
        bodyDetails.add(tie);

        // 3. Shoulder Epaulets with Golden Stars
        const epL = new Mesh(new BoxGeo(0.13, 0.025, 0.20), navyMat);
        epL.position.set(-0.35, 0.34, 0.05);
        epL.rotation.z = -0.32;
        bodyDetails.add(epL);

        const epStarL = new Mesh(new OctahedronGeo(0.025, 0), goldMat);
        epStarL.position.set(-0.37, 0.36, 0.05);
        bodyDetails.add(epStarL);

        const epR = new Mesh(new BoxGeo(0.13, 0.025, 0.20), navyMat);
        epR.position.set(0.35, 0.34, 0.05);
        epR.rotation.z = 0.32;
        bodyDetails.add(epR);

        const epStarR = new Mesh(new OctahedronGeo(0.025, 0), goldMat);
        epStarR.position.set(0.37, 0.36, 0.05);
        bodyDetails.add(epStarR);

        // 4. Walkie-Talkie on Left Shoulder
        const radio = new Mesh(new BoxGeo(0.06, 0.11, 0.05), blackGlossMat);
        radio.position.set(-0.31, 0.42, 0.07);
        radio.rotation.z = -0.15;
        bodyDetails.add(radio);

        const antenna = new Mesh(new CylinderGeo(0.006, 0.006, 0.09, 8), blackGlossMat);
        antenna.position.set(-0.32, 0.51, 0.07);
        bodyDetails.add(antenna);

        // 5. Police Duty Belt with Silver Buckle
        const belt = new Mesh(new TorusGeo(0.43, 0.028, 12, 32), blackGlossMat);
        belt.position.set(0, -0.14, 0.02);
        belt.rotation.x = Math.PI / 2;
        bodyDetails.add(belt);

        const buckle = new Mesh(new BoxGeo(0.08, 0.06, 0.02), silverMat);
        buckle.position.set(0, -0.14, 0.45);
        bodyDetails.add(buckle);

        // 6. Mini Police Baton (Matraque) on Right Hip
        const baton = new Mesh(new CylinderGeo(0.015, 0.015, 0.24, 8), blackGlossMat);
        baton.position.set(0.42, -0.16, 0.05);
  ...
  // Position camera for a crisp front beauty shot
  await send('Runtime.evaluate', {
    expression: `(() => {
      const cam = window.game.engine.camera;
      cam.position.set(0, 1.8, 3.4);
      cam.lookAt(0, 1.3, 0);
    })()`
  });
  ...
  fs.writeFileSync(path.join(outDir, 'officer_skin_clean.png'), Buffer.from(shot.data, 'base64'));
  console.log('Saved officer_skin_clean.png successfully! Total errors:', uncaughtErrors.length);
